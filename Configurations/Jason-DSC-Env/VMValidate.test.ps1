...
It "[S1] Should have an IP address of 192.168.3.51" {
    $i = Invoke-command -ScriptBlock { Get-NetIPAddress -interfacealias 'Ethernet' -AddressFamily IPv4} -Session $S1
...
It "[S1] Should have an IP address of 192.168.3.70" {
...
$dns = icm {Get-DnsClientServerAddress -InterfaceAlias ethernet -AddressFamily IPv4} -session $PullServer
It "[S1] Should have a DNS server configuration of 192.168.3.70" {                        
  $dns.ServerAddresses -contains '192.168.3.70' | Should Be "True"           
}
}
